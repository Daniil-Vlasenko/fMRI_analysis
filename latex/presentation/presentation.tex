...
\begin{document}
	...
	\begin{frame} 
		...
		$\{(r_i^n, r_j^n)\}_n$ --- множество пар значений вершин $v_i^n$, $v_j^n$ из выборки $(\widetilde{X}, \widetilde{Y})$.
		
		\begin{prob_def}
			$w_{ij}^k = P(y_k = \rom{2} | r_i^k, r_j^k, \{(r_i^n, r_j^n)\}_n) - P(y_k = \rom{1} | r_i^k, r_j^k, \{(r_i^n, r_j^n)\}_n)$
		\end{prob_def}				
		
		Пусть $Cl: \{y_k |(r_i^k, r_j^k), \{(r_i^n, r_j^n)\}_n\}_k \rightarrow [0, 1]$ --- вероятностный классификатор.
		
		\begin{algo_def}			
			\begin{equation*}
				\begin{multlined}
					w_{ij}^k = Cl(y_k = \rom{2} | (r_i^k, r_j^k), \{(r_i^n, r_j^n)\}_n) - \\ - Cl(y_k = \rom{1} | (r_i^k, r_j^k), \{(r_i^n, r_j^n)\}_n)
				\end{multlined}
			\end{equation*}
		\end{algo_def}	
	\end{frame}
